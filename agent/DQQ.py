...
from keras.layers import Input, Dense, Lambda, Add, Flatten, Conv1D
...
def OurModel(input_shape, action_space, dueling):
    ...
    X = Conv1D(64, 5, strides=(3),padding="valid", input_shape=input_shape, activation="relu", data_format="channels_first")(X)
    X = Conv1D(64, 4, strides=(2),padding="valid", activation="relu", data_format="channels_first")(X)
    X = Conv1D(64, 3, strides=(1),padding="valid", activation="relu", data_format="channels_first")(X)
    X = Flatten()(X)
    ...
    model = Model(inputs = X_input, outputs = X, name='abc')
