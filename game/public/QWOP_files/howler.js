...
(function() {
    ...
    r.prototype = {
        ...
        play: function(a, b) {
            ...
            c._inactiveNode(function(d) {
                ...
                var e = 0 < d._pos ? d._pos : c._sprite[a][0] / 1E3
                  , f = 0;
                c._webAudio ? (f = c._sprite[a][1] / 1E3 - d._pos,
                0 < d._pos && (e = c._sprite[a][0] / 1E3 + e)) : f = c._sprite[a][1] / 1E3 - (e - c._sprite[a][0] / 1E3);
                var h = !(!c._loop && !c._sprite[a][2]), m = "string" === typeof b ? b : Math.round(Date.now() * Math.random()) + "", l;
                (function() {
                    l = setTimeout(function() {
                        !c._webAudio && h && c.stop(m).play(a, m);
                        c._webAudio && !h && (c._nodeById(m).paused = !0,
        ...
        loop: function(a) {
            return "boolean" === typeof a ? (this._loop = a,
            this) : this._loop
        },
        sprite: function(a) {
            return "object" === typeof a ? (this._sprite = a,
            this) : this._sprite
        },
        pos: function(a, b) {
            var c = this;
            ...
            var d = b ? c._nodeById(b) : c._activeNode();
            if (d)
                return 0 <= a ? (c.pause(b),
                d._pos = a,
                c.play(d._sprite, b),
                c) : c._webAudio ? d._pos + (g.currentTime - c._playStart) : d.currentTime;
            if (0 <= a)
                return c;
        ...
        rate: function(a, b) {
            ...
            if (0 <= a && 1E3 >= a) {
                if (c._webAudio) {
                    var d = b ? c._nodeById(b) : c._activeNode();
                    d && (c._rate = a,
                    d.bufferSource.playbackRate.value = a)
                }
            } else
                return c._rate;
            return c
        ...
        fade: function(a, b, c, d, e) {
            ...
            for (var k = 1; k <= g; k++)
                (function() {
                    var a = Math.round(1E3 * (f._volume + ("up" === h ? .01 : -.01) * k)) / 1E3;
                    setTimeout(function() {
                        f.volume(a, e);
                        a === b && d && d()
                    }, l * k)
        ...
        _drainPool: function() {
            ...
            for (b = this._audioNode.length - 1; 0 <= b && !(5 >= a); b--)
                this._audioNode[b].paused && (this._webAudio && this._audioNode[b].disconnect(0),
                a--,
    ...
    if (l)
        var x = function(a, b) {
          ...
          , w = function(a, b, c) {
            g.decodeAudioData(a, function(a) {
                a && (n[c] = a,
                v(b, a))
            }, function(a) {
                b.on("loaderror")
          ...
          , y = function(a, b, c) {
            ...
            b[0] && (c.bufferSource.loopStart = b[1],
            c.bufferSource.loopEnd = b[1] + b[2]);
    ...
    exports.Howl = r);
    "undefined" !== typeof window && (window.Howler = k,
    window.Howl = r)
}
)();
